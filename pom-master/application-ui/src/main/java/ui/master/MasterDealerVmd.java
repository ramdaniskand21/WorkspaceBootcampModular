...
public class MasterDealerVmd extends BaseVmd implements Serializable {
	...
	public void setMstDealerDtoSelected(MstDealerDto mstDealerDtoSelected) {
		this.mstDealerDtoSelected = mstDealerDtoSelected;
	}
	...
	protected void loadList(){
		...
		try {
			indexDtos = JsonUtil.mapJsonToListObject(rest.getContents(), MstDealerDto.class);
	...
	private List<MstGlobalParamDto> getMstGlobalParam(String condition){
		...
		RestResponse rest = callWs("/MasterGlobalParam/all/paging/"+condition, null, HttpMethod.GET);
		try{
			out = JsonUtil.mapJsonToListObject(rest.getContents(),MstGlobalParamDto.class );
		}catch(Exception e){
			e.printStackTrace();
		}
		return out;
	...
	public void groupDealerHandler(
		...
		BindUtils.postNotifyChange(null, null, formMaster, "groupDealerCode");
	}
	...
	@Command
	public void edit(){
		if(mstDealerDtoSelected == null || mstDealerDtoSelected.getDealerCode()==null){
			showSmartMsgBox("W001");
			return;
		}
		
		Executions.getCurrent().setAttribute("obj", mstDealerDtoSelected);
	...
	@Override
	public void onEdit(){
		mstDealerDto = (MstDealerDto) Executions.getCurrent().getAttribute("obj");
		BindUtils.postNotifyChange(null, null, this, "mstDealerDto");
	}
